  ...
  },
  {
    toJSON: {
      ...
      transform: (doc, rec, options) => {
        ...
        if (rec?.attributes.fitType)
          rec.attributes.fitType = rec.attributes.fitType[lang];
        if (rec?.attributes.careInstructions)
          rec.attributes.careInstructions =
            rec.attributes.careInstructions[lang];
        if (rec?.gender) rec.gender = rec.gender[lang];
        if (rec?.season) rec.season = rec.season[lang];
      },
    },
  },
);
